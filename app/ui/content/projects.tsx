export default function Projects(){
    return (
        <section id="projects" className="h-128h m-auto pt-32 w-2/5 min-w-80 min-h-2xl">
            <h2 className="text-3xl font-medium text-center"><span className="text-green">02. </span>Projects</h2>
            <div className="w-10/12 h-32 border-gray-500 rounded border m-auto my-10 bg-white-2 shadow-glow flex justify-evenly">
                <div className="w-1/4 text-center">
                    Capstone
                </div>
                <div className="w-3/4 text-center">
                    Description
                </div>
            </div>
            <div className="w-10/12 h-32 border-gray-500 rounded border m-auto my-10 bg-white-2 shadow-glow">

            </div>
            <div className="w-10/12 h-32 border-gray-500 rounded border m-auto my-10 bg-white-2 shadow-glow">

            </div>

        </section>
    )
}